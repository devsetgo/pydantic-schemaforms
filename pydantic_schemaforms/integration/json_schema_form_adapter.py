"""JSON Schema data adapter for react-jsonschema-form consumers.

This module generates JSON Schema + UI Schema configuration blobs that a
frontend developer wires into a react-jsonschema-form (or compatible)
component. It does NOT render HTML or produce React components.
"""
...
class ReactJSONSchemaFormAdapter:
    """Produce JSON Schema artifacts for react-jsonschema-form consumers.

    Outputs ``schema``, ``uiSchema``, and ``formData`` dicts that a frontend
    developer passes to a react-jsonschema-form component. No HTML is produced;
    no React components are generated.
    """
...
__all__ = ['ReactJSONSchemaFormAdapter']
