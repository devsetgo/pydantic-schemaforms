...
from .base import SelectInputBase, FormInput, build_label, build_error_message, build_help_text
...
class SelectInput(SelectInputBase):
    ...
    valid_attributes = SelectInputBase.valid_attributes + [
        "size", "multiple", "autofocus"
    ]
    ...
    def _build_options(self, options: List[Dict[str, Any]]) -> str:
        ...
        for option in options:
            if isinstance(option, dict):
                ...
                option_parts.append(f'<option value="{escape(str(option))}">{escape(str(option))}</option>')
...
class CheckboxGroup(SelectInputBase):
    ...
    def render(self, options: List[Dict[str, Any]], group_name: str, 
               legend: Optional[str] = None, **kwargs) -> str:
