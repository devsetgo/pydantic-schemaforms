...
class TestModelListUncovered:
    def _item_model(self):
        class ItemModel(FormModel):
            name: str = 'default'

        return ItemModel

    def test_render_with_min_items_and_empty_values(self):
        from pydantic_schemaforms.model_list import ModelListRenderer

        ItemModel = self._item_model()
    ...
    def test_render_material_framework(self):
        from pydantic_schemaforms.model_list import ModelListRenderer
        ...
        assert 'items' in html or 'hello' in html

    def test_render_with_underscore_field_skipped(self):
        from pydantic_schemaforms.model_list import ModelListRenderer

        ItemModel = self._item_model()
        renderer = ModelListRenderer(framework='bootstrap')

        # Patch model_json_schema to include a _hidden field
        original = ItemModel.model_json_schema.__func__

        @classmethod  # type: ignore[misc]
        def patched_schema(cls, **kwargs):
            schema = original(cls, **kwargs)
            schema.setdefault('properties', {})['_hidden'] = {'type': 'string'}
            return schema

        ItemModel.model_json_schema = patched_schema
        try:
            html = renderer.render_model_list(
                field_name='items',
                label='Items',
            ...
            assert '_hidden' not in html
        finally:
            del ItemModel.model_json_schema

    def test_render_material_with_underscore_field_skipped(self):
        from pydantic_schemaforms.model_list import ModelListRenderer
        ...
        def patched_schema(cls, **kwargs):
            schema = original(cls, **kwargs)
            schema.setdefault('properties', {})['_priv'] = {'type': 'string'}
            return schema
        ...
        try:
            ...
            assert '_priv' not in html
        finally:
            del ItemModel.model_json_schema

    def test_render_themed_container_fallback(self):
        from pydantic_schemaforms.model_list import ModelListRenderer
        ...
        renderer._resolve_theme = lambda: EmptyTheme()
        html = renderer.render_model_list(
            field_name='items',
            label='Items',
            model_class=ItemModel,
