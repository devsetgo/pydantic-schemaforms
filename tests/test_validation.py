...
class TestFormModelValidation:
    ...
    def test_custom_validator_functions(self):
        ...
        form = CustomValidationForm(
            username="user123", password="Password123", confirm_password="Password123"
        )
        ...
        with pytest.raises(ValidationError):
            CustomValidationForm(
                username="user@123", password="Password123", confirm_password="Password123"
            )

        # Test invalid password (too short)
        with pytest.raises(ValidationError):
            CustomValidationForm(username="user123", password="Pass1", confirm_password="Pass1")
        ...
        with pytest.raises(ValidationError):
            CustomValidationForm(
                username="user123", password="Password", confirm_password="Password"
            )
...
class TestValidationIntegration:
    ...
    def test_validation_with_form_rendering(self, simple_form_model, enhanced_renderer):
        ...
        html = enhanced_renderer.render_form_from_model(simple_form_model, errors=result.errors)
...
class TestValidationPerformance:
    ...
    def test_large_form_validation(self):
        ...
        valid_data = {
            f"field{i}": (
                f"value{i}" if i <= 5 else (i if i <= 8 else (float(i) if i == 9 else True))
            )
            for i in range(1, 11)
        }
