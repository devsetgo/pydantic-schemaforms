...
from pydantic_forms.schema_form import (
    ...
    TabbedLayout,
    VerticalLayout,
    form_validator,FormField, EmailStr
)
...
class SimpleForm(FormModel):
    title: str = FormField(..., title="Title", input_type="text", placeholder="Enter title here", help_text="The title of the item.", icon="bi bi-card-text")
    age: int = FormField(..., title="Age", input_type="number", min=0, max=120, help_text="Your age in years.", icon="bi bi-person")
    email: EmailStr = FormField(..., title="Email", input_type="email", placeholder="example@example.com", help_text="Your email address.", icon="bi bi-envelope")
    subscribe: bool = FormField(False, title="Subscribe to newsletter", input_type="checkbox", help_text="Check to receive our newsletter.", icon="bi bi-newspaper")
    country: str = FormField(..., title="Country", input_type="select", options=["USA", "Canada", "UK", "Australia"], help_text="Select your country of residence.", icon="bi bi-globe")
    birth_date: date = FormField(..., title="Birth Date", input_type="date", help_text="Your date of birth.", icon="bi bi-calendar")
    appointment_time: datetime = FormField(None, title="Appointment Time", type="datetime", help_text="Preferred time for appointment.", icon="bi bi-clock")
    credit_card_number: str = FormField(..., title="Credit Card Number", type="credit_card", placeholder="1234 5678 9012 3456", help_text="Enter your credit card number.", icon="bi bi-credit-card")
...
class UserProfile(FormModel):
    bio: str = FormField(None, title="Biography", input_type="textarea", placeholder="Tell us about yourself", help_text="A brief biography.", icon="bi bi-info-circle")
    profile_picture: str = FormField(None, title="Profile Picture", type="file", help_text="Upload your profile picture.", icon="bi bi-image")



class LayoutOne(VerticalLayout):
    form: SimpleForm # layout with simple form is a vertical layout
    form_config = SectionDesign(section_title="User Profile Section", 
...
class LayoutTwo(HorizontalLayout):
    ...
    form_config = SectionDesign(section_title="Additional Profile Details", 
...
class UserProfileLayout(TabbedLayout):
    ...
    form_config = FormDesign(ui_theme="material", # UI framework (bootstrap, material, shadcn, tailwind, semantic, custom)
