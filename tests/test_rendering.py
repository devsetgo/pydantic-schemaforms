...
def test_enhanced_renderer_branches_and_wrappers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        renderer, "_render_layout_fields_as_tabs", lambda *_args: ["layout-tabs"]
    )
    monkeypatch.setattr(
    ...
    assert (
        renderer_wrappers._render_layout_field_content("n", {}, None, None, {}, SimpleNamespace())
        == "Y"
    )
...
def test_form_layouts_vertical_horizontal_header_and_exception_branches(
    ...
    monkeypatch.setattr(
        vertical, "_render_form_instances", lambda **_kwargs: ["<div>Body</div>"]
    )
    ...
    monkeypatch.setattr(
        horizontal, "_render_form_instances", lambda **_kwargs: ["<div>Col</div>"]
    )
