...
4. **Document unified validation engine** — Add short guide/examples showing how `ValidationResponse`, convenience validators, and `ValidationSchema.build_live_validator()` are used across sync + HTMX flows; include cross-field examples.
5. **Automated E2E coverage for layouts/async** — Extend test suite to include tabs, accordions, and async renderer paths; add a smoke test for the Layout Demonstration page (Bootstrap/Material) to assert initial tab content renders and tab switching works headlessly.
