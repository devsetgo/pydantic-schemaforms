...
from .base import NumericInput, build_label, build_error_message, build_help_text
...
class ScoreInput(NumberInput):
    ...
    def render(self, min_score: Union[int, float] = 0, max_score: Union[int, float] = 100, **kwargs) -> str:
        kwargs["min"] = str(min_score)
        kwargs["max"] = str(max_score)
        kwargs["step"] = kwargs.get("step", "1" if isinstance(min_score, int) and isinstance(max_score, int) else "0.1")
